"""Test with damping to match Python script"""
import numpy as np
...
print(f"Test with damping={damping}")
print("="*60)
...
for it in range(500):
    ...
    for N in range(1, N_max+1):
        for x in x_values:
            mu_new = profit(N, x) + Psi1[(N, x)]
            mu_cutoff[(N, x)] = damping * mu_new + (1 - damping) * mu_old[(N, x)]
            
            if N < N_max:
                gamma_new = Psi2.get((N, x), 0.0)
                gamma_cutoff[(N, x)] = damping * gamma_new + (1 - damping) * gamma_old[(N, x)]
    
    # Update V̄ using NEW cutoffs WITH damping
    for N in range(1, N_max+1):
        for x in x_values:
            thresh = mu_cutoff[(N, x)]
            z = (thresh - mu_bar) / sigma_mu
            
            if z > 10:
                V_new = mu_bar + sigma_mu * norm.pdf(z) / 1e-12
            elif z < -10:
                V_new = thresh
            else:
                Phi_z = norm.cdf(z)
                ...
                if Phi_z < 0.9999:
                    ...
                    exit_term = 0.0
                V_new = stay_term + exit_term
            
            V_bar[(N, x)] = damping * V_new + (1 - damping) * V_old[(N, x)]
