"""
Dynamic Entry/Exit Game - CORRECTED Implementation
Economics 600a - Yale University, Fall 2025
Marek Chadim

CORRECTIONS:
1. Fixed forward simulation for incumbents - properly handles period 0 conditioning
2. Clarified CCP matching logic with detailed comments
3. Fixed timing of entry/exit decisions per homework specification
...
class BBLEstimator:
    """BBL estimation for dynamic entry/exit game - CORRECTED VERSION."""
    ...
    def forward_simulate_incumbent(self, N_init, x_init, mu, sigma_mu, 
        ...
        """
        Forward simulate PDV for one incumbent conditional on STAYING in period 0.
        
        CRITICAL: Per HW Question 9 Step B:
        "Λ(N,x) = E[PDV | Firm 1 STAYS in period 0, then follows d̂(N,x) thereafter]"
        
        This means:
        - Period 0: Firm 1 earns profit π(N,x) (we condition on staying)
        - Period 0: OTHER firms may exit based on their draws
        - Period 0: Entry may occur
        - Period 1+: Firm 1 follows d̂(N,x) like everyone else
        """
        max_periods = 100
        N_t = N_init
        x_t = x_init
        ...
        # PERIOD 0: Firm 1 earns profit (conditioned on staying)
        pdv = self.compute_pi(N_t, x_t)
        
        # PERIOD 0: Simulate exit decisions for OTHER incumbents (not Firm 1)
        n_others = N_t - 1
        n_stay_others = 0
        ...
        if n_others > 0:
            for i in range(n_others):
                ...
                # CCP LOGIC: Firm stays if Φ((μ_it - μ)/σ_μ) ≤ d̂(N,x)
                # This means: firms with lower exit values (bottom d̂ percentile) stay
                prob_mu = norm.cdf((mu_it - mu) / sigma_mu)
                
                if prob_mu <= self.d_hat.get((N_t, x_t), 0.5):
                    n_stay_others += 1
        
        # After period 0 exits, N = staying others + Firm 1
        N_t = n_stay_others + 1
        
        # PERIOD 0: Simulate entry decision
        if N_t < self.N_max:
            gamma_it = gamma + sigma_gamma * self.gamma_draws_std[state_idx, sim_idx, 0]
            # CCP LOGIC: Entrant enters if Φ((γ_it - γ)/σ_γ) ≤ ê(N,x)
            # This means: entrants with lower costs (bottom ê percentile) enter
            prob_gamma = norm.cdf((gamma_it - gamma) / sigma_gamma)
            
            if prob_gamma <= self.e_hat.get((N_t, x_t), 0.5):
                N_t += 1
        
        # PERIOD 0: Transition demand shifter
        ...
        for t in range(1, max_periods):
            ...
            if n_others > 0:
                for i in range(n_others):
                    mu_it = mu + sigma_mu * self.mu_draws_std[state_idx, sim_idx, t, i + 1]
                    prob_mu = norm.cdf((mu_it - mu) / sigma_mu)
                    
                    if prob_mu <= self.d_hat.get((N_t, x_t), 0.5):
                        n_stay_others += 1
            
            N_t = n_stay_others + 1  # +1 for Firm 1 who is still alive
    ...
    def forward_simulate_entrant(self, N_init, x_init, mu, sigma_mu,
                                gamma, sigma_gamma, state_idx, sim_idx):
        """
        Forward simulate PDV for potential entrant ENTERING in period 0.
        
        Per HW: "If the firm decides to enter at t, their single period payoffs at t 
        are −γ_it - i.e. they only begin to produce and make profits in the next period."
        
        So Λ^E(N,x) is the expected PDV net of entry cost γ_it.
        """
        max_periods = 100
        pdv = 0.0  # No profit in period 0 (entry cost not included in Λ^E)
        
        N_t = N_init
        x_t = x_init
        x_idx = np.where(self.x_values == x_t)[0][0]
        
        # PERIOD 0: Existing incumbents may exit
        n_stay = 0
        if N_t > 0:
            for i in range(N_t):
                mu_it = mu + sigma_mu * self.mu_draws_std[state_idx, sim_idx, 0, i]
        ...
        # After entry: N = staying incumbents + 1 (the entrant)
        N_t = n_stay + 1
        ...
        # PERIODS 1+: Now entrant is an incumbent, follows d̂(N,x)
        focal_firm_alive = True
        ...
        for t in range(1, max_periods):
            ...
            mu_entrant = mu + sigma_mu * self.mu_draws_std[state_idx, sim_idx, t, 0]
...
if __name__ == "__main__":
    print("="*70)
    print("Dynamic Entry/Exit Game - CORRECTED Implementation")
    print("="*70)
    ...
    # BBL Estimation
    print("\n" + "="*70)
    print("BBL Estimation (CORRECTED)")
    print("="*70)
